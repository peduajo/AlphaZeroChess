...
alphazero_params = SimpleNamespace(**{
    'run_name':             'alphazero_connect4_chess',
...
class AlphaZero:
    def __init__(self, net, writer, optimizer):
        ...
        self.criterion_policy = nn.CrossEntropyLoss(reduction='none')
        self.criterion_value = nn.MSELoss(reduction='none')

        self.quantization_dataloader = None
        self.batch_size_quantization = alphazero_params.batch_quant
    ...
    def add_data_to_buffer(self, data_path):
        ...
        values_path_sample = f"{data_path}values/0.zip"
        ...
        custom_dataset_quant = model.CustomDataset(states_path_sample, policy_path_sample, values_path_sample, quant=True)
    ...
    def export_to_jit(self):
        torch.save(self.model.state_dict(), f"models/experiment_1/model_{self.iteration}.pt")
        torch.save(self.optimizer.state_dict(), f"models/experiment_1/optimizer_{self.iteration}.pt")
        ...
        if self.iteration == 0:
            example = torch.rand(1, 119, 8, 8).to(self.model.device)
            traced_script_module = torch.jit.trace(self.model, example)

            torch.jit.save(traced_script_module, model_path2)

            compile_spec = {"inputs": [torch_tensorrt.Input(min_shape=[1, 119, 8, 8],
                                                            opt_shape=[self.batch_size_quantization, 119, 8, 8],
                                                            max_shape=[2048, 119, 8, 8])],
            ...
            compile_spec = {
                    "inputs": [torch_tensorrt.Input(min_shape=[1, 119, 8, 8],
                                                    opt_shape=[self.batch_size_quantization, 119, 8, 8],
                                                    max_shape=[2048, 119, 8, 8])],
    ...
    def drop_duplicates(self, memory):
        ...
        print(f"media de jugadas por partida: {avg_games}")
    ...
    def train(self):
        ...
        for i in range(n_steps):
            ...
            state, policy_targets, _, value_targets = batch
            ...
            #se para el entrenamiento si la política ha cambiado mucho ya en esta fase de entrenamiento
            with torch.no_grad():
...
net = model.ResNet(alphazero_params.resnet_num_blocks, alphazero_params.resnet_hidden, 8, 8, 8*8*73, 119, device=device)
