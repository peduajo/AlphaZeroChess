...
class ResNet(nn.Module):
    def __init__(self, num_resBlocks, num_hidden, row_count, column_count, action_size, planes_size, device=torch.device("cuda")):
        super().__init__()
        self.device = device 
        self.startBlock = nn.Sequential(
            nn.Conv2d(planes_size, num_hidden, kernel_size=3, padding=1),
        ...
        self.policyHead = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(64 * row_count * column_count, action_size)
        )
